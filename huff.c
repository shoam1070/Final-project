...
char *argv[];
{
    ...
    printf ("It took you %.2lf seconds to Decode.\n, Or %f", dif, cpu_time_used);
    free( (char *) nodes );
}
 ...
 */
int build_tree( nodes1 )
NODE *nodes1;
{
    int next_free, next_free1;
    int i;
    int min_1;
    int min_2;
    NODE nodes[ 514 ];
	...
	nodes[ i ].count = nodes1[ i ].count;
	nodes[ i ].saved_count = 0;
	nodes[ i ].value = i;
	nodes[ i ].level = 0;
    }
    ...
    find_level ( nodes, next_free, 0 );
    merge_sort ( nodes );
    for ( i = 0 ; i <= END_OF_STREAM ; i++ )
        memcpy ( nodes1 + nodes [ i ].value , nodes + i , sizeof ( NODE ) );
    for ( i = END_OF_STREAM + 1 ; i < 513 ; i++ )
	nodes1[ i ].saved_count = 0;
    for ( i = 0 ; nodes[ i ].level == 0 ; i++ );
    next_free1 = END_OF_STREAM + 1;
    canonical ( nodes, nodes1, &next_free1, &i, END_OF_STREAM + 1, 1 );
    return( END_OF_STREAM + 1 );
}

canonical ( nodes, nodes1, next_free, i, n, level)
NODE *nodes, *nodes1;
int *next_free, *i, n, level;
{

    if ( level == nodes[ *i ].level ) {
	nodes1[ n ].child_0 = nodes[ *i ].value;
	nodes1[ n ].saved_count += nodes1[ nodes [ *i ].value ].saved_count;
	( *i )++;
    }
    else {
	( *next_free )++;
	nodes1[ n ].child_0 = *next_free;
	canonical ( nodes, nodes1, next_free, i, *next_free, level + 1 );
	nodes1[ n ].saved_count += nodes1[ nodes1[ n ].child_0 ].saved_count;
    }
    if ( level == nodes[ *i ].level ) {
	nodes1[ n ].child_1 = nodes[ *i ].value;
	nodes1[ n ].saved_count += nodes1[ nodes [ *i ].value ].saved_count;
	( *i )++;
    }
    else {
	( *next_free )++;
	nodes1[ n ].child_1 = *next_free;
	canonical ( nodes, nodes1, next_free, i, *next_free, level + 1 );
	nodes1[ n ].saved_count += nodes1[ nodes1[ n ].child_1 ].saved_count;
    }
}
	
find_level ( nodes, n, level )
NODE *nodes;
int n, level;
{
    if ( n > END_OF_STREAM ) {
	level++;
	find_level ( nodes, nodes[ n ].child_0, level );
	find_level ( nodes, nodes[ n ].child_1, level );
    }
    else {
	nodes[ n ].level = level;
...
NODE *x;
{
    int lb1,lb2,lb3,ub1,ub2,i,j,k,size=1;
    NODE aux[ END_OF_STREAM+1 ];

    while ( size <= END_OF_STREAM )
    {
        lb1=lb3=0;
        while ( lb1+size <= END_OF_STREAM )
        {
            lb2=lb1+size;
            ub1=lb2-1;
            if ( ub1+size > END_OF_STREAM )
                ub2=END_OF_STREAM;
            else
                ub2=ub1+size;
            i=lb1;
            ...
            k=lb3;
            while (( i <= ub1 ) && ( j <= ub2 ))
                if ( x[i].level < x[j].level )
                    memcpy ( aux + k++ , x + i++ , sizeof ( NODE ) );
                else
                    memcpy ( aux + k++ , x + j++ , sizeof ( NODE ) );
            while ( i <= ub1 )
                memcpy ( aux + k++ , x + i++ , sizeof ( NODE ) );
            while ( j <= ub2 )
                memcpy ( aux + k++ , x + j++ , sizeof ( NODE ) );
        ...
        i=lb1;
        while ( lb3 <= END_OF_STREAM )
            memcpy ( aux + lb3++ , x + i++ , sizeof ( NODE ) );
        for ( k=0 ; k<=END_OF_STREAM ; k++ )
            memcpy ( x + k , aux + k , sizeof ( NODE ) );
        size*=2;
    }
}

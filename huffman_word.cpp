...
void printCodes(struct MinHeapNode* root, string str, std::map<std::string,std::string> &mymap)
{
...
////////////////////////////////////////////////////////////////////////
    ...
    std::vector<std::string > results((std::istream_iterator<std::string>(iss)), // results is words vector
    std::istream_iterator<std::string>());
    map <std::string , int  > words;      // words is a map with word (key) and frequency (val)
    for (size_t i = 0; i < results.size(); i++) {
      if (words.find(results[i]) == words.end())
          words[results[i]] = 1;
      else words.find(results[i])->second +=1;
    }
...
// //        std::cout << "god = " << words.find("god")->second<< '\n';
